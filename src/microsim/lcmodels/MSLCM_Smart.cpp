...
/// @date    Fri, 08.10.2013
/// @version $Id: MSLCM_Smart.cpp 26003 2017-09-13 12:04:00Z namdre $
///
...
// ===========================================================================
MSLCM_Smart::MSLCM_Smart(MSVehicle& v) :
MSAbstractLaneChangeModel(v, LCM_SMART),
...
bool
MSLCM_Smart::debugVehicle() const {
	return DEBUG_COND;
}

LaneChangeModel MSLCM_Smart::getModelID() const {
	return LCM_SMART;
}

MSVehicle* MSLCM_Smart::getFollowerGroupLeader(const MSLane& neighLane, double myPosOnLane){
	...
	while (ok && vehs.size()!=0){
		...
		if ((*i)->getPositionOnLane() < myPosOnLane
			&& getMessengerDeviceFromVehicle(*i)->getLeader() == *i &&
				getMessengerDeviceFromVehicle(*i)->iMustPass != true) {
...
MSVehicle** firstBlocked) {
	...
	if (myOwnState & LCA_AMBACKBLOCKER) getMessengerDeviceFromVehicle(&myVehicle)->iMustPass = true;
	...
	if (!leading || (libsumo::Simulation::getCurrentTime()/1000 - leadStartTime<3 && getMessengerDeviceFromVehicle(&myVehicle)->getGroupSize()==0))
		...
		if (required.result==0){
            ...
            if (leading && required.result!=0 && followerGroupLeader!=NULL){
                if (myVehicle.getEdge() == followerGroupLeader->getEdge() &&
                        getMessengerDeviceFromVehicle(&myVehicle)->getGroupLength()+10 < myVehicle.getPositionOnLane(myVehicle.getLane())-followerGroupLeader->getPositionOnLane(followerGroupLeader->getLane())){
...
double
MSLCM_Smart::patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
...
double
MSLCM_Smart::_patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
...
void*
MSLCM_Smart::inform(void* info, MSVehicle* sender) {
...
double
MSLCM_Smart::overtakeDistance(const MSVehicle* follower, const MSVehicle* leader, const double gap, double followerSpeed, double leaderSpeed) {
...
double
MSLCM_Smart::informLeader(MSAbstractLaneChangeModel::MSLCMessager& msgPass,
int blocked,
int dir,
const std::pair<MSVehicle*, double>& neighLead,
...
void
MSLCM_Smart::informFollower(MSAbstractLaneChangeModel::MSLCMessager& msgPass,
int blocked,
int dir,
const std::pair<MSVehicle*, double>& neighFollow,
double remainingSeconds,
...
MSLCM_Smart::changed() {
	...
	try {
		if (smartLeader != NULL) {
			...
			if (leading) getMessengerDeviceFromVehicle(&myVehicle)->notifyMemberLC();
		}
	} catch (...) {}
...
MSVehicle** firstBlocked) {
    ...
    if (requested == 0){
        ...
        return laneOffset==sentByLeader.laneOffset?sentByLeader.result:0;
    }
...
double
MSLCM_Smart::roundaboutDistBonus(double roundaboutDistAhead, int roundaboutEdgesAhead) const {
...
double
MSLCM_Smart::distanceAlongNextRoundabout(double position, const MSLane* initialLane, const std::vector<MSLane*>& continuationLanes) {
...
void
MSLCM_Smart::saveBlockerLength(MSVehicle* blocker, int lcaCounter) {
...
void
MSLCM_Smart::adaptSpeedToPedestrians(const MSLane* lane, double& v) {
...
std::string
MSLCM_Smart::getParameter(const std::string& key) const {
...
void
MSLCM_Smart::setParameter(const std::string& key, const std::string& value) {
	double doubleValue;
	try {
...
bool MSLCM_Smart::isLeading() const {
	return leading;
}

void MSLCM_Smart::setLeading(bool leading) {
	std::cout << name << " leading set to: " << leading << std::endl;
